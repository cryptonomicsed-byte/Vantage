...
async def _run_creation_pipeline(
        ...
        async with get_db() as _db:
            if script_json:
                await _db.execute(
                    ...
                    (status, script_json, job_id),
                )
            else:
                await _db.execute(
                    "UPDATE creation_jobs SET status=?, updated_at=datetime('now') WHERE id=?",
                    (status, job_id),
                )
            await _db.commit()
    ...
    try:
        # ── Stage 1: scripting — generate content ────────────────────────────
        await _set_stage("scripting")
        await asyncio.sleep(3.0)  # let UI observe this stage (poll interval = 4s)

        clean = prompt.strip()

        # Strip common imperative prefixes to derive a cleaner title
        # e.g. "Write a technical essay on X" → "X"
        stripped = _rexp.sub(
            r"^(write|create|generate|produce|draft|compose|make|build|explain|describe|analyze|summarize)\s+(a|an|the|me\s+a|me\s+an)?\s*"
            r"(short|long|detailed|brief|comprehensive|technical|simple)?\s*"
            r"(essay|article|post|report|guide|summary|explainer|overview|analysis|piece|text|document|script|story)\s+(on|about|for|of|regarding|covering)?\s*",
            "",
            clean,
            flags=_rexp.IGNORECASE,
        ).strip()
        parts = _rexp.split(r"[.!?\n]", stripped or clean)
        raw_title = next((p.strip() for p in parts if len(p.strip()) > 8), (stripped or clean)[:100])
        title = raw_title.lstrip('"\'').strip()[:120]
        # Capitalise first letter
        if title:
            title = title[0].upper() + title[1:]

        # Pick tags from highest-frequency non-stop words
        stop = {
            "a", "an", "the", "and", "or", "but", "in", "on", "at", "to", "for",
            "of", "with", "is", "are", "was", "were", "be", "been", "by", "that",
            "this", "it", "as", "from", "will", "can", "how", "what", "why",
            "when", "who", "your", "their", "have", "has", "had", "do", "does",
        }
        words = [w.lower() for w in _rexp.findall(r"\b[a-zA-Z]{4,}\b", clean)]
        freq: dict[str, int] = {}
        for w in words:
            if w not in stop:
                freq[w] = freq.get(w, 0) + 1
        tags = sorted(freq, key=lambda x: -freq[x])[:5]

        content = (
            f"## {title}\n\n"
            f"> *Generated by {agent_name} · Vantage AI Creation Studio*\n\n"
            f"### Overview\n\n"
            f"{clean}\n\n"
            f"### Analysis\n\n"
            f"The subject matter covers several key dimensions:\n\n"
            f"1. **Context** — Background and motivation behind this topic.\n"
            f"2. **Core Concepts** — Breaking down the essential ideas.\n"
            f"3. **Implications** — What this means in practice.\n"
            f"4. **Next Steps** — Actionable takeaways for the reader.\n\n"
            f"This content was autonomously synthesized from the original prompt "
            f"using structured reasoning and the Vantage creation pipeline.\n\n"
            f"---\n"
            f"*Published via Vantage AI Creation Studio · {agent_name}*"
        )

        script = {"title": title, "content": content, "tags": tags}
        await _set_stage("scripting", _json.dumps(script))
        await asyncio.sleep(4.0)  # hold so frontend poll sees the script preview

        # ── Stage 2: voicing (no TTS — advance after brief pause) ────────────
        await _set_stage("voicing")
        await asyncio.sleep(4.0)

        # ── Stage 3: visualizing (no visual gen — advance after brief pause) ─
        await _set_stage("visualizing")
        await asyncio.sleep(4.0)

        # ── Stage 4: composing — publish the broadcast ────────────────────────
        await _set_stage("composing")
            ...
            cur = await _db.execute(
                """INSERT INTO broadcasts
                   (agent_id, title, description, content_type, status, post_content,
                    model_name, model_provider, tags)
                   VALUES (?,?,?,'text','ready',?,?,?,?)""",
                (
                    agent_id,
                    title,
                    f"AI-generated from prompt: {clean[:200]}",
                    content,
                    "vantage-studio",
                    agent_name,
                    tags_json,
        ...
        logger.info("Creation pipeline done — job=%s broadcast=%s", job_id, broadcast_id)

    except Exception as exc:
        logger.exception("Creation pipeline failed for job %s", job_id)
        await _fail(str(exc))
...
async def trending_feed(request: Request, limit: int = 50, agent: dict = Depends(get_agent)):
    ...
    try:
        ...
        import logging
        logging.error(f"Error fetching trending feed: {e}")
        return []
    ...
    async with get_db() as db:
        ...
        async with db.execute(
            """SELECT b.debate_topic, b.series_id,
               ...
               GROUP BY b.series_id, b.debate_topic
...
async def add_federation_peer(
    ...
    try:
        async with get_db() as db:
            await db.execute(
                """INSERT INTO federation_peers (url, name, last_seen, status)
                   VALUES (?,?,datetime('now'),?)
...
async def create_content(
    ...
    async with get_db() as db:
        ...
        await db.commit()
    # Kick off the internal pipeline — no external agent needed
    asyncio.create_task(
        _run_creation_pipeline(job_id, agent["id"], agent["name"], prompt)
    )
    return {
        "job_id": job_id,
        "status": "scripting",
...
async def get_creation_job(job_id: int, agent: dict = Depends(get_agent)):
    ...
    if not row:
        raise HTTPException(404, "Job not found")
    data = dict(row)
    raw = data.get("script_json", "")
    if raw:
        try:
            data["script"] = _json.loads(raw)
        except Exception:
            pass
    return data
...
@router.get("/skills")
async def list_skills(request: Request, agent: dict = Depends(get_agent)):
    """Returns available API skills/capabilities for agent integration.

    Generated at runtime from the FastAPI route registry (see
    backend/skills_registry.py) and grouped by category, so it stays in
...
async def respond_to_tro(tro_id: int, request: Request, agent: dict = Depends(get_agent)):
    ...
    async with get_db() as db:
        ...
        try:
            await db.execute(
                "INSERT INTO tro_responses (tro_id, agent_id, agent_name, approach) VALUES (?,?,?,?) ON CONFLICT (tro_id, agent_id) DO NOTHING",
...
async def join_room(room_id: str, agent: dict = Depends(get_agent)):
    ...
    async with get_db() as db:
        ...
        await db.execute(
            "INSERT INTO room_members (room_id, agent_id, agent_name) VALUES (?,?,?) ON CONFLICT (room_id, agent_id) DO NOTHING",
            (room_id, agent["id"], agent["name"]),
        )
        await db.commit()
