...
from backend.backtest.loaders.rsshub_events import (
    FeedSpec,
    RSSHubEventProvider,
    enrich_price_frames_with_events,
    feed_specs_from_config,
)
from backend.backtest.loaders.tushare_fundamentals import (
...
from backend.backtest.models import EquitySnapshot, Position, TradeRecord
...
class BaseEngine(ABC):
    ...
    def run_backtest(
        ...
        if bench_ticker and bench_ticker != "auto":
            from backend.backtest.benchmark import resolve_benchmark
        ...
        if config.get("validation"):
            from backend.backtest.validation import run_validation, write_validation_json
        ...
        from backend.backtest.run_card import write_run_card
        write_run_card(
            run_dir,
            config,
            m,
