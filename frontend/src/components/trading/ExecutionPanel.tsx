...
// ── Generate Wallet — POST /api/trading/wallets/generate, already real and
// working on the backend (both 'bip39' — the vanity-real/Cloakseed-family
// multi-chain generator on port 8778 — and 'bipon39' — the Yoruba Ifá
// wordlist system, /opt/ares/ares_bip39.py + the `bipon39` CLI) but had no
// UI anywhere. The mnemonic is shown here exactly once, straight from the
// API response — the backend never stores it, only the encrypted private
// key. Cloakseed itself (vanity-cloakseed) is a separate, standalone
// client-side app for turning a real seed phrase into a stealth cipher
// for physical backup — it has no server API, so it can't be called from
// here, but any mnemonic generated below is a real BIP-39-shaped phrase
// you can paste into it yourself if you want that extra layer.
function GenerateWalletModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.walletSection}>
        ...
        {showGenerateWallet && (
          <GenerateWalletModal onClose={() => setShowGenerateWallet(false)} onCreated={refreshWallets} />
        )}
