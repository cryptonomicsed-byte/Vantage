...
const SUGGESTIONS = [
  ...
  'Check my PnL',
  '/pine RSI with overbought/oversold zones',
  'Go to trading',
  '/help',
]
...
const HELP_TEXT = [
  ...
  '/pine <description> — generate a Pine Script indicator and save it to your library',
...
async function runSlashCommand(query: string, apiKey: string): Promise<string> {
  ...
  if (cmd === 'pine') {
    if (!rest) return 'Usage: /pine <describe the indicator> — e.g. /pine RSI divergence with volume confirmation'
    try {
      const gen = await fetch('/api/pine/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Agent-Key': apiKey },
        body: JSON.stringify({ prompt: rest, symbol: 'BTC', interval: '1h' }),
      })
      const gd = await gen.json().catch(() => ({}))
      if (!gen.ok || !gd.script) return `Pine generation failed: ${gd.detail || gen.statusText}`
      // Save to the agent's library so it shows up in Terminal → Pine → My Scripts.
      const scriptName = rest.slice(0, 40).replace(/["'\\]/g, '').trim() || 'Copilot Indicator'
      let savedNote = ''
      try {
        const sv = await fetch('/api/pine/indicators', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-Agent-Key': apiKey },
          body: JSON.stringify({ name: scriptName, script: gd.script, description: `Generated by Copilot: ${rest}` }),
        })
        if (sv.ok) savedNote = `\n\nSaved to your library as "${scriptName}" — open Trading → Terminal → Pine → My Scripts to run it on the chart.`
      } catch {}
      const via = gd.method === 'llm' ? 'LLM' : 'template'
      return `Generated Pine Script (${via}):\n\n${gd.script}${savedNote}`
    } catch { return 'Network error generating Pine Script.' }
  }
