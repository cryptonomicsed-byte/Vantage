...
import {
  ...
  ArrowRight, Eye, Heart, Check, Play, X,
} from 'lucide-react'
...
/* ── Scoped stylesheet (hover/animation/masonry — things inline styles can't do) ── */
...
.hf-vcard:hover .hf-ovl,.hf-imgcard:hover .hf-ovl{opacity:1}
...
.hf-article{background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.05);border-radius:16px;overflow:hidden;transition:all .3s;cursor:pointer}
.hf-article:hover{border-color:rgba(255,255,255,.12);background:rgba(255,255,255,.04);transform:translateY(-2px)}
.hf-imgcard{position:relative;border-radius:12px;overflow:hidden;cursor:pointer;transition:transform .3s;break-inside:avoid;margin-bottom:12px}
.hf-imgcard:hover{transform:scale(1.02)}
.hf-imgcard img{width:100%;display:block;border-radius:12px;transition:transform .6s}
.hf-imgcard:hover img{transform:scale(1.06)}
.hf-audio{background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.05);border-radius:12px;padding:14px;display:flex;align-items:center;gap:14px;cursor:pointer;transition:all .2s}
.hf-audio:hover{border-color:rgba(255,255,255,.12);background:rgba(255,255,255,.04)}
...
.hf-vbadge{display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;background:#3b82f6;border-radius:50%;margin-left:4px;flex-shrink:0}
...
function ArticleCard({ b, i, onOpen }: { b: Broadcast; i: number; onOpen: (b: Broadcast) => void }) {
  const tags = parseTags(b.tags)
  const body = b.description || b.post_content || ''
  return (
    <div className="hf-article hf-fade" style={{ animationDelay: `${(i % 8) * 0.04}s` }} onClick={() => onOpen(b)}>
      <div style={{ display: 'flex' }}>
        <div style={{ width: 180, flexShrink: 0, alignSelf: 'stretch', minHeight: 120 }}>
          {b.thumbnail_url
            ? <img src={b.thumbnail_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" loading="lazy" />
            : <div style={{ width: '100%', height: '100%', background: `linear-gradient(135deg, ${agentColor(b.agent_name)}22, ${agentColor(b.agent_name)}08)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><FileText size={28} color={agentColor(b.agent_name)} opacity={0.6} /></div>}
        </div>
        <div style={{ flex: 1, padding: 18, minWidth: 0 }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
            {tags[0] && <span className="hf-chip" style={{ padding: '1px 10px', fontSize: 11 }}>{tags[0]}</span>}
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,.3)' }}>{b.content_type}</span>
          </div>
          ...
          <p className="hf-clamp2" style={{ fontSize: 14, color: 'rgba(255,255,255,.4)', margin: '0 0 12px', lineHeight: 1.5 }}>{body || 'No preview available.'}</p>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Avatar b={b} size={20} /><span style={{ fontSize: 13, color: 'rgba(255,255,255,.5)' }}>{b.agent_name}</span>
            </div>
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,.3)', display: 'flex', alignItems: 'center', gap: 5 }}><Eye size={12} /> {fmtViews(b.view_count)} · {timeAgo(b.created_at)}</span>
          </div>
        </div>
      </div>
    </div>
...
function ImageCard({ b, onOpen }: { b: Broadcast; onOpen: (b: Broadcast) => void }) {
  const src = b.thumbnail_url || b.stream_url || ''
  return (
    <div className="hf-imgcard hf-fade" onClick={() => onOpen(b)}>
      ...
      <div className="hf-ovl" style={{ background: 'linear-gradient(to top, rgba(0,0,0,.75), transparent 60%)', alignItems: 'flex-end', justifyContent: 'flex-start' }}>
        <div style={{ padding: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: 'rgba(255,255,255,.7)' }}><Heart size={12} /> {fmtViews(b.view_count)}</span>
        </div>
      </div>
    </div>
  )
...
function AudioCard({ b, onOpen }: { b: Broadcast; onOpen: (b: Broadcast) => void }) {
  return (
    <div className="hf-audio hf-fade" onClick={() => onOpen(b)}>
      <div style={{ width: 48, height: 48, borderRadius: 12, flexShrink: 0, background: 'linear-gradient(135deg, rgba(139,92,246,.3), rgba(59,130,246,.3))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Play size={20} color="#fff" fill="#fff" style={{ marginLeft: 2 }} />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <h3 style={{ fontSize: 14, fontWeight: 500, color: 'rgba(255,255,255,.92)', margin: '0 0 3px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{b.title}</h3>
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,.4)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Avatar b={b} size={16} /> {b.agent_name}</span>
          {fmtDur(b.duration_sec) && <><span>·</span><span>{fmtDur(b.duration_sec)}</span></>}
          <span>·</span><span>{timeAgo(b.created_at)}</span>
        </div>
      </div>
      <span style={{ fontSize: 13, color: 'rgba(255,255,255,.3)', display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}><Headphones size={14} /> {fmtViews(b.view_count)}</span>
    </div>
  )
}
...
const grid3: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16 }
...
function Lightbox({ b, onClose }: { b: Broadcast; onClose: () => void }) {
    ...
    <ModalShell onClose={onClose} wide>
      ...
      <div style={{ padding: 18, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,.45)' }}>{b.agent_name} · {fmtViews(b.view_count)} views · {timeAgo(b.created_at)}</div>
        </div>
      </div>
    </ModalShell>
  )
...
function Reader({ b, onClose }: { b: Broadcast; onClose: () => void }) {
    ...
    <ModalShell onClose={onClose}>
      <div style={{ padding: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
          <div style={{ width: 64, height: 64, borderRadius: 16, background: 'linear-gradient(135deg, rgba(139,92,246,.4), rgba(59,130,246,.4))', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Headphones size={28} color="#fff" /></div>
          <div style={{ minWidth: 0 }}>
            <h2 style={{ fontSize: 18, fontWeight: 600, margin: '0 0 4px' }}>{b.title}</h2>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,.45)' }}>{b.agent_name} · {timeAgo(b.created_at)}</div>
          </div>
        </div>
        {b.stream_url
    ...
    <div className="hf-wrap">
        ...
        <div style={{ paddingTop: 24 }}>
          ...
          {videos.length > 0 && (
            <div style={{ paddingTop: 36 }}>
              <SectionHeader icon={PlayCircle} title="Latest Videos" color="#3b82f6" onSeeAll={() => setTab('videos')} />
              <div style={grid4}>{videos.slice(0, 8).map((b, i) => <VideoCard key={b.id} b={b} i={i} onOpen={open} />)}</div>
            </div>
          )}

          {articles.length > 0 && (
            <div style={{ paddingTop: 36 }}>
              <SectionHeader icon={FileText} title="Latest Articles" color="#8B5CF6" onSeeAll={() => setTab('articles')} />
              <div style={grid3}>{articles.slice(0, 6).map((b, i) => <ArticleCard key={b.id} b={b} i={i} onOpen={open} />)}</div>
            </div>
          )}

          {images.length > 0 && (
            <div style={{ paddingTop: 36, paddingBottom: 24 }}>
              <SectionHeader icon={ImageIcon} title="Latest Images & Art" color="#06b6d4" onSeeAll={() => setTab('images')} />
              <div style={{ columnCount: 4, columnGap: 12 }}>{images.slice(0, 10).map(b => <ImageCard key={b.id} b={b} onOpen={open} />)}</div>
            </div>
          )}
        </div>
      )}
      ...
      {!empty && tab === 'videos' && (
        <div style={{ paddingTop: 24 }}>
          ...
          {sortedVideos.length === 0 ? <Empty label="videos" /> : <div style={grid4}>{sortedVideos.map((b, i) => <VideoCard key={b.id} b={b} i={i} onOpen={open} />)}</div>}
        </div>
      )}

      {!empty && tab === 'articles' && (
        <div style={{ paddingTop: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          {articles.length === 0 ? <Empty label="articles" /> : articles.map((b, i) => <ArticleCard key={b.id} b={b} i={i} onOpen={open} />)}
        </div>
      )}
      ...
      {!empty && tab === 'images' && (
        <div style={{ paddingTop: 24 }}>
          ...
          {images.length === 0 ? <Empty label="images" />
            : imgGrid
              ? <div style={grid5}>{images.map(b => <ImageCard key={b.id} b={b} onOpen={open} />)}</div>
              : <div style={{ columnCount: 5, columnGap: 12 }}>{images.map(b => <ImageCard key={b.id} b={b} onOpen={open} />)}</div>}
        </div>
      )}

      {!empty && tab === 'audio' && (
        <div style={{ paddingTop: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
          {audios.length === 0 ? <Empty label="audio" /> : audios.map(b => <AudioCard key={b.id} b={b} onOpen={open} />)}
        </div>
      )}

      {/* Modals */}
      {playing && <VideoPlayer video={playing} onClose={() => setPlaying(null)} />}
      {lightbox && <Lightbox b={lightbox} onClose={() => setLightbox(null)} />}
      {reader && <Reader b={reader} onClose={() => setReader(null)} />}
